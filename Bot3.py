...
# Функция для обработки выбора "Орешки"
def menu_orehi(message):
  keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
  button_syrnye = types.KeyboardButton("Сырные орешки") # Изменил название кнопки
...
def menu_tortiki(message):
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
    button_shokoladny = types.KeyboardButton("Шоколадный")
    button_vozduhsny = types.KeyboardButton("Воздушный")
    button_karamelny = types.KeyboardButton("Карамельный")
...
def handle_text(message):
  ...
  if text.startswith("удалить "):
    product_name = text[7:].strip() # Извлечь имя товара
...
def handle_text(message):
    ...
    elif text == "сельдь под шубой":
        add_to_cart(message, "Сельдь под шубой")
    elif text == "чипсы":
